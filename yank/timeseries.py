...
#=============================================================================================
def statisticalInefficiency(A_n, B_n=None, fast=True, mintime=3):
  """
  ...
  # Trap the case where this covariance is zero; return g = 1.
  if(sigma2_AB == 0):
    return 1.0
  ...
  while (t < N-1):
    ...
    C = ( dA_n[0:(N-t)]*dB_n[t:N] + dB_n[0:(N-t)]*dA_n[t:N] ).sum() / (2.0 * float(N-t) * sigma2_AB)
  ...
  return g
#=============================================================================================
def statisticalInefficiencyMultiple(A_kn, fast=True, return_correlation_function=False):
  """
  ...
  return g
#=============================================================================================
def integratedAutocorrelationTime(A_n, B_n=None, fast=True, mintime=3):
  """
  ...
  return tau
#=============================================================================================
def integratedAutocorrelationTimeMultiple(A_kn, fast=True):
  """
...
def normalizedFluctuationCorrelationFunction(A_n, B_n=None, N_max=None):
  ...
  for t in range(0,N_max+1):
    # compute normalized fluctuation correlation function at time t
    C_n[t] = ( dA_n[0:(N-t)]*dB_n[t:N] + dB_n[0:(N-t)]*dA_n[t:N] ).sum() / (2.0 * float(N-t) * sigma2_AB)
...
def detectEquilibration(A_t):
    ...
    Neff_max (int) - number of uncorrelated samples   
    indices (list of int) - indices of effectively uncorrelated samples
    
    NOTES
    
    This function estimates the effective number of uncorrelated samples for all subsets [t0,T]
    where the initial time t0 ranges over [0,T] and the final time T is fixed.  The equilibrated
    region returned is the one that maximizes the effective number of uncorrelated samples.

    WARNING

    This function may be slow for very long timeseries.
    ...
    indices = subsampleCorrelatedData(A_t[t:T], g=g) + t
    Neff_max = indices.size
    
    return (t, g, Neff_max, indices)
