...
class SetupDatabase:
    ...
    def is_molecule_setup(self, molecule_id):
        ...
        elif molecule_descr['parameters'] == 'antechamber':
            # Get paths of important files
            molecule_dir = self.get_molecule_dir(molecule_id)
            parameters = os.path.join(molecule_dir, molecule_id + '.frcmod')
            file_path = os.path.join(molecule_dir, molecule_id + '.gaff.mol2')

            # Check that files exist
            if not (os.path.isfile(parameters) and os.path.getsize(parameters) > 0 and
                        os.path.isfile(file_path) and os.path.getsize(file_path) > 0):
                is_setup = False
            else:  # Make sure internal description is correct
                is_processed = True
                molecule_descr['filepath'] = file_path
                molecule_descr['parameters'] = parameters
        ...
        # If a single structure must be extracted we search for output
            ...
            single_molecule_file = os.path.join(molecule_dir, molecule_id + extension)
            if not (os.path.isfile(single_molecule_file) and
                            os.path.getsize(single_molecule_file) > 0):
                is_setup = False
            else:  # Make sure internal description is correct
                is_processed = True
                molecule_descr['filepath'] = single_molecule_file
    ...
    def get_system(self, components):
    ...
    def _setup_molecules(self, *args):
        ...
        for mol_id in args:
            ...
            if 'filepath' not in mol_descr:
                ...
                self._pos_cache[mol_id] = utils.get_oe_mol_positions(oe_molecules[mol_id])
            ...
            if mol_id in oe_molecules:
                ...
                openmoltools.openeye.molecule_to_mol2(oe_molecules[mol_id], mol_descr['filepath'],
            ...
            if 'select' in mol_descr:
                ...
                if extension == '.pdb':
                    ...
                    # Cache positions for later and save new filepath
                    mol_descr['filepath'] = single_pdb_path
                    self._pos_cache[mol_id] = pdb_file.getPositions(asNumpy=True, frame=model_idx) / unit.angstrom
                else:
                    raise RuntimeError('Model selection is not supported for file {}'.format(extension[1:]))
